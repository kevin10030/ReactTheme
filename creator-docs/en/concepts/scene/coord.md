...
> **Note**: the [rotation](https://docs.cocos.com/creator/3.0/api/en/classes/scene_graph.node-1.html#rotation) property on a node is a quaternion that represents the angle of rotation about any axis. The property corresponding to `Rotation` in the **Inspector** is the property [EulerAngles](https://docs.cocos.com/creator/3.0/api/en/classes/scene_graph.node-1.html#eulerangles). These two properties can be used separately according to your needs. When using the API, make sure to pay attention to the difference between them and the editor panel property names.
